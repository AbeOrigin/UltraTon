...
import type {
  SecureUltraTonRequestOptions,
  UltraTonRequestOptions,
} from "./types/request-options.types.ts";
...
import { parseSecurePayload, secureReviver } from "./helpers/payload-parser.ts";
...
export class UltraTonClient {
   ...
   */
  constructor(
    transportInterceptor?: typeof https.request | typeof http.request,
  ) {
  ...
  public async get<T = unknown>(
    url: string,
    options?: Omit<UltraTonRequestOptions, "method" | "body">,
  ): Promise<UltraTonResponse<T>> {
  ...
  private _consumeResponseBuffer(
    ...
    return new Promise((resolve, reject) => {
        ...
        if (currentBodySize > reqOpts.maxBodySize) {
          ...
          reject(
            new UltraTonMemoryError(
              `UltraTon: Body size exceeded the maximum allowed size of ${reqOpts.maxBodySize} bytes`,
            ),
          );
          return;
        }
      ...
      res.on("aborted", () => {
        ...
        reject(
          new SecureHttpError(
            "UltraTon: The connection was abruptly aborted by the server.",
          ),
        );
      });
      ...
      res.on("close", () => {
        ...
        reject(
          new SecureHttpError(
            "UltraTon: The incoming network stream was closed prematurely.",
          ),
        );
      });
  ...
  private _executeSingleNetworkHop(
    ...
    return new Promise((resolve, reject) => {
      ...
      const transport =
        this.#transport ||
        (urlObj.protocol === "http:" ? http.request : https.request);
      ...
      if (currentTimeoutMs > 0) {
        executionTimer = setTimeout(() => {
          req.destroy();
          reject(
            new UltraTonNetworkTimeoutError(
              `UltraTon: Absolute execution time of ${reqOpts.timeoutMs}ms exceeded.`,
            ),
          );
        }, currentTimeoutMs);
      }
      ...
      req.on("response", (res: IncomingMessage) => {
        const isRedirect =
          ...
          HTTP_CODES_REDIRECTS.has(res.statusCode);
        ...
        this._consumeResponseBuffer(
          res,
          req as ClientRequest,
          reqOpts,
          executionTimer,
        )
          .then((data) => {
      ...
      req.on("timeout", () => {
        ...
        reject(
          new UltraTonNetworkTimeoutError(
            `UltraTon: Socket completely idle for ${reqOpts.socketTimeoutMs}ms.`,
          ),
        );
      });
  ...
  private async _request<T = unknown>(
    ...
    while (true) {
      ...
      if (reqOpts.timeoutMs > 0) {
        ...
        if (currentTimeoutMs === 0) {
          throw new UltraTonNetworkTimeoutError(
            `UltraTon: Absolute execution time of ${reqOpts.timeoutMs}ms exceeded.`,
          );
        }
      }

      const hop = await this._executeSingleNetworkHop(
        urlObj,
        reqOpts,
        currentOptions,
        currentTimeoutMs,
      ...
      if (
        ...
        typeof hop.headers.location === "string"
      ) {
        if (reqOpts.maxRedirects > 0) {
          if (redirectCount >= reqOpts.maxRedirects) {
            throw new UltraTonRedirectError(
              `UltraTon: Max redirects (${reqOpts.maxRedirects}) exceeded.`,
            );
          }
          ...
          const nextHeaders = { ...reqOpts.headers } as Record<
            string,
          ...
          if (
            hop.statusCode === 303 ||
            ((hop.statusCode === 301 || hop.statusCode === 302) &&
              nextMethod !== "GET" &&
              nextMethod !== "HEAD")
          ) {
            nextMethod = "GET";
      ...
      const parsedBody = parseSecurePayload<T>(
        hop.data,
      ...
      return {
        ...
        json(): T {
          try {
            // If body is already parsed (not a Buffer), return it as-is
            if (!(parsedBody instanceof Buffer)) {
              return parsedBody as T;
            }
            // If body is a Buffer, parse it
            if (hop.data.length === 0) return {} as T;
            return JSON.parse(parsedBody.toString("utf-8"), secureReviver) as T;
          } catch (e: unknown) {
